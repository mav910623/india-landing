...
export default function DashboardPage() {
  ...
  /** ===== Progress caches ===== */
  const [l1Progress, setL1Progress] = useState({});   // uid -> [0..10]
  const [l2Leaders10, setL2Leaders10] = useState(0);  // # of L2 with ≥10
  ...
  async function toggleNode(parentUid, level) {
    ...
    try {
      ...
      console.error(e);
      setTreeError(t("errors.expandCollapse"));
    } finally {
  ...
  function heroLine() {
    if (!mission1Done) {
      ...
      return (
        <>
          {t.rich("hero.m1.prefix", { strong: (c) => <strong>{c}</strong> })}{" "}
          <strong>10</strong>{" "}
          {t.rich("hero.m1.suffix", {
            need,
            strong: (c) => <strong className="text-blue-700 font-semibold">{c}</strong>,
          })}
        </>
      );
    }
      ...
      <main className="mx-auto max-w-4xl px-4 py-6 sm:py-8">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="shrink-0" id="qrShareBlock">
              <div
                ...
                title={t("qr.inviteTitle")}
              >
                <div className="text-[11px] font-medium text-gray-600 mb-2">{t("qr.inviteLabel")}</div>
        ...
        <section className="mt-6">
          <div className="grid gap-3 sm:grid-cols-2">
            <MissionWide
              title={t("missions.m1.title")}
              subtitle={t("missions.m1.subtitle")}
            ...
            {(showAllMissions || ((counts.levels?.["1"] || 0) >= L1_GOAL)) && (
              <MissionWide
                title={t("missions.m2.title")}
                subtitle={t("missions.m2.subtitle")}
            ...
            {(showAllMissions || ( (counts.levels?.["1"] || 0) >= L1_GOAL && completedL1 >= M2_TARGET)) && (
              <MissionWide
                title={t("missions.m3.title")}
                subtitle={t("missions.m3.subtitle")}
          ...
          <div className="mt-3 flex items-center justify-center">
            <button
              ...
              title={showAllMissions ? t("missions.hideAll") : t("missions.viewAll")}
            >
              {showAllMissions ? t("missions.hide") : t("missions.view")}
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <h3 className="text-lg font-semibold text-gray-900">{t("team.title")}</h3>
              <button
                ...
                title={showLevelBreakdown ? t("team.hideBreakdown") : t("team.showBreakdown")}
              >
                <span className="font-semibold">{t("team.total", { n: counts.total })}</span>
                <span className={`transition-transform ${showLevelBreakdown ? "rotate-180" : ""}`}>
                  ▼
                </span>
              </button>
            </div>
            {treeLoading && <span className="text-xs text-gray-500">{t("loading")}</span>}
          </div>
          ...
          <div className="mt-3">
            <div className="flex items-center gap-2 mb-2">
              <button
                ...
                title={expanded.has(currentUid) ? t("team.collapse") : t("team.expand")}
              >
          ...
          <div className="absolute inset-x-0 bottom-0 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 w-full sm:w-[560px]">
            <div className="rounded-t-2xl sm:rounded-2xl border border-gray-100 bg-white shadow-xl p-4 sm:p-5">
              <div className="flex items-center justify-between">
                <h4 className="text-base font-semibold text-gray-900">{t("helpTitle")}</h4>
                <button
                  onClick={() => setShowHelp(false)}
    ...
    return (
      <div className="relative">
        ...
        {hasMore && (
          ...
          >
            {/** i18n not needed here; tiny helper */}
            Loading more…
          </div>
        )}
      </div>
